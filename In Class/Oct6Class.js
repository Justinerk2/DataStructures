...
MongoClient.connect(url, function(err, db) {
    ...
    // collection.aggregate([{$group : { _id: null, "averageNumberStations" : { $avg : "$totalDocks"} } }]).toArray(function(err, docs) {
...
// collection.aggregate([{$match : {availableBikes : 0}}, {$group : {_id : null, count : {$sum: 1}}}]).toArray(function(err, docs) {
collection.aggregate([{$match : {availableBikes : 0}}, {$group : {_id : null, count : {$sum: 1}, averageDocks : {$avg : "$totalDocks"}}}]).toArray(function(err, docs) {
